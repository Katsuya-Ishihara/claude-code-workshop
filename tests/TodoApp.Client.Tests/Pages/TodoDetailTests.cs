...
public class TodoDetailTests : TestContext
{
    ...
    public TodoDetailTests()
    {
        ...
        _userApiClientMock.Setup(x => x.GetUsersAsync(It.IsAny<CancellationToken>()))
    ...
    public void Todo詳細情報が正しく表示される()
    {
        ...
        _todoApiClientMock.Setup(x => x.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(todo);
    ...
    public async Task ステータス変更ボタンをクリックするとステータスが更新される()
    {
        ...
        _todoApiClientMock.Setup(x => x.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(todo);
        _todoApiClientMock.Setup(x => x.UpdateStatusAsync(1, It.Is<UpdateTodoStatusRequest>(r => r.Status == TodoStatus.InProgress), It.IsAny<CancellationToken>()))
        ...
        // Assert
        _todoApiClientMock.Verify(x => x.UpdateStatusAsync(1, It.Is<UpdateTodoStatusRequest>(r => r.Status == TodoStatus.InProgress), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task InProgressからCompletedへステータス遷移する()
    {
        ...
        _todoApiClientMock.Setup(x => x.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(todo);
        _todoApiClientMock.Setup(x => x.UpdateStatusAsync(1, It.Is<UpdateTodoStatusRequest>(r => r.Status == TodoStatus.Completed), It.IsAny<CancellationToken>()))
            .ReturnsAsync(updatedTodo);
        ...
        // Assert
        _todoApiClientMock.Verify(x => x.UpdateStatusAsync(1, It.Is<UpdateTodoStatusRequest>(r => r.Status == TodoStatus.Completed), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task 削除確認後にTodoが削除される()
    {
        ...
        var todo = CreateSampleTodo();
        _todoApiClientMock.Setup(x => x.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(todo);
        _todoApiClientMock.Setup(x => x.DeleteAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(true);
        ...
        // Assert
        _todoApiClientMock.Verify(x => x.DeleteAsync(1, It.IsAny<CancellationToken>()), Times.Once);
    ...
    {
        // Arrange
        _todoApiClientMock.Setup(x => x.GetByIdAsync(999, It.IsAny<CancellationToken>())).ReturnsAsync((TodoResponse?)null);
    ...
    public void ローディング中はスピナーが表示される()
    {
        ...
        _todoApiClientMock.Setup(x => x.GetByIdAsync(1, It.IsAny<CancellationToken>())).Returns(tcs.Task);

        // Also need to handle users loading
        var usersTcs = new TaskCompletionSource<List<UserResponse>>();
        _userApiClientMock.Setup(x => x.GetUsersAsync(It.IsAny<CancellationToken>())).Returns(usersTcs.Task);
