...
public class TodoListTests : TestContext
{
    ...
    public void ローディング中はスピナーが表示される()
    {
        ...
        _mockTodoApi.Setup(x => x.GetAllAsync()).Returns(tcs.Task);
        _mockUserApi.Setup(x => x.GetUsersAsync()).Returns(new TaskCompletionSource<List<UserResponse>>().Task);
    ...
    public void Todo一覧が正しく表示される()
    {
        ...
        _mockTodoApi.Setup(x => x.GetAllAsync()).ReturnsAsync(todos);
        _mockUserApi.Setup(x => x.GetUsersAsync()).ReturnsAsync(users);
    ...
    {
        // Arrange
        _mockTodoApi.Setup(x => x.GetAllAsync()).ReturnsAsync(new List<TodoResponse>());
        _mockUserApi.Setup(x => x.GetUsersAsync()).ReturnsAsync(new List<UserResponse>());
    ...
    {
        // Arrange
        _mockTodoApi.Setup(x => x.GetAllAsync()).ThrowsAsync(new HttpRequestException("API error"));
        _mockUserApi.Setup(x => x.GetUsersAsync()).ReturnsAsync(new List<UserResponse>());
