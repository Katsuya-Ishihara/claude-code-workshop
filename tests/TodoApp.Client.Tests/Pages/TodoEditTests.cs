...
public class TodoEditTests : TestContext
{
    ...
    public void ページタイトルが表示される()
    {
        ...
        _mockTodoApi.Setup(x => x.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(todo);
    ...
    public void 更新成功時にTodo詳細ページに遷移する()
    {
        ...
        _mockTodoApi.Setup(x => x.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(todo);
        _mockTodoApi
            .Setup(x => x.UpdateAsync(1, It.IsAny<UpdateTodoRequest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(todo);
    ...
    {
        // Arrange
        _mockTodoApi.Setup(x => x.GetByIdAsync(999, It.IsAny<CancellationToken>())).ReturnsAsync((TodoResponse?)null);
